(function productExperienceFeature() {
  ...
  function pxRenderProductExecutionRoadmap(
    ...
    if (!renderer || typeof renderer.render !== "function") {
      return `
        <p class="product-experience-empty-copy">
          No está disponible el módulo de ejecución del producto.
        </p>
      `;
    }
    ...
    return renderer.render({
      ...
      countryId,
      capabilities,
      internalItems: internalRoadmapItems,
      featureRows: pxRows("jiraWorkspaceFeatures"),
  ...
  function pxEnsureProductExecutionFeatures(productId, countryId) {
    ...
    renderer
      .ensureFeatures({
        ...
        featureRows: pxRows("jiraWorkspaceFeatures"),
        load:
          typeof loadJiraFeaturesData === "function"
            ? loadJiraFeaturesData
            : null,
        install:
          typeof installJiraFeaturesData === "function"
            ? installJiraFeaturesData
            : null,
      })
      .then((loaded) => {
        if (!loaded) {
  ...
  function pxRenderLocalProduct(productId, countryId) {
    ...
    const executionRoadmap = pxRenderProductExecutionRoadmap(
      productId,
      normalizedCountryId,
      capabilities,
      internalRoadmapItems,
    ...
    view.innerHTML = `
      ...
      >
        <button
          class="ghost-button navigation-back-button product-experience-back"
          type="button"
        ...
        <section class="product-experience-overview-panel">
          ...
          <aside class="product-experience-product-summary">
            <article>
              <span>País</span>
              <strong>${pxEsc(countryLabel)}</strong>
            </article>
            <article>
            ...
            <article>
              <span>Elementos roadmap</span>
              <strong>${internalRoadmapItems.length}</strong>
            </article>
          </aside>
        </section>
        ...
        <section class="product-experience-section">
          <header class="product-experience-section-header">
            <div>
              <span class="product-experience-eyebrow">Capacidades</span>
              <h2>
                Qué contiene ${pxEsc(product.productName)} en ${pxEsc(countryLabel)}
              </h2>
            </div>
            <p>
              Definición global del producto y situación de ejecución de cada
          ...
          <div class="product-experience-capability-grid">
            ${
              capabilities.length
                ...
                : `
                    <p class="product-experience-empty-copy">
                      No hay capacidades informadas para este producto.
                    </p>
                  `
            }
          </div>
        ...
        <section class="product-experience-section product-experience-timeline-summary">
          <header class="product-experience-section-header product-experience-timeline-header">
            <div>
              <span class="product-experience-eyebrow">Resumen del plan</span>
              <h2>${pxEsc(product.productName)} · ${pxEsc(countryLabel)}</h2>
            </div>
            <p>
              ${internalRoadmapItems.length} elementos internos/MSA ·
              ${linkedItems.length} vinculados a capacidad ·
              ${unassignedItems} sin capacidad ·
              ${averageProgress}% avance medio ·
              ${riskCount} en riesgo.
            </p>
          </header>
